...
void setup_aesni_128_key(const struct xombuf *xbuf, const unsigned char *key) {
    ...
    memcpy(*(unsigned char **) xbuf + key_offset_lo, key, AES_128_CTR_KEY_SIZE >> 1);
    memcpy(*(unsigned char **) xbuf + key_offset_hi, key + (AES_128_CTR_KEY_SIZE >> 1), AES_128_CTR_KEY_SIZE >> 1);
}
